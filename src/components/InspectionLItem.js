...
const InspectionLItem = ({date, inspection, count,index, dateInspectionScreen}) => {
  return (
    <View className=" p-1 border-b-2 border-b-haskoyOrange">
        ...
        <Text  style={{fontFamily:"serif"}}>{typeof(count) == 'string' ? (count.charAt(0).toUpperCase() + count.slice(1)):(count) }</Text>
        </TouchableOpacity>
    </View>
  )
} 
