...
const StudentAnalyzeScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    const callPastInspection = async () => {
      ...
      await setanalyzeResult(analyze(pastInspectionsData.data));
    ...
    <SafeAreaView>
      ...
      <View className="items-center mt-48">
        <AnimatedCircularProgress
          size={200}
          width={15}
          fill={(parseFloat("0." + analyzeResult[3]) * 100) == 10 ? 100 : (parseFloat("0." + analyzeResult[3]) * 100)}
          tintColor="orange"
        ...
        <Text className="font-bold text-midnight mt-5 text-2xl">
          Programlara Katılım Oranı %{parseInt(analyzeResult[3])}
        </Text>
      </View>
    </SafeAreaView>
  );
